...
echo base64_encode(base64_encode($cek["username"]).",".base64_encode($cek["dcname"]));
die();
}
else if($cek["pass"]==$key && $cek["discordid"]==$disid && $cek["ip"]!=getUserIP() && $cek["mailto"] == $email)
{
     die("Username Error : V-U-1");
}
else if($cek["pass"]==$key && $cek["discordid"]!=$disid && $cek["ip"]==getUserIP() && $cek["mailto"] == $email)

{
    die("Username Error : V-U-2");
}
else if($cek["pass"]!=$key && $cek["discordid"]==$disid && $cek["ip"]==getUserIP() && $cek["mailto"] == $email)
{
    die("Username Error : V-U-3");
}
else if($cek["pass"]==$key && $cek["discordid"]==$disid && $cek["ip"]==getUserIP() && $cek["mailto"] != $email)
{
    die("Username Error : V-U-4");
}
